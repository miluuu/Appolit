...
optimizer_choice = 1 # 0 scipy, 1 gridsearch, else knitro
...
algo = API.Algorithm(instance, max_iter, episode_max, VF_approx_architecture, optimizer_choice)

algo.approximate_policy_iteration(instance, dataset, dataset_type)


#solution_storage = np.zeros(t_max)
#solution_storage = algo.evaluate_policy(instance, dataset_type + ' datasets/' + dataset + '/txt/e.txt', dataset_type + ' datasets/' + dataset + '/txt/p.txt', dataset_type + ' datasets/' + dataset + '/txt/D.txt')
#for k in range(number_samplepaths):
#    outputfcts.plot_data_and_save(instance, solution_storage[k], 'R' + '-' + dataset, 'Results/' + dataset + '/solution_storage plot, ' + dataset + ', ' + str(max_iter) + ' iterations, m = ' + str(episode_max) + ', sample ' + str(k) + ' .pdf')
#    outputfcts.save_data_in_file(solution_storage[k], 'Results/' + dataset + '/solution_storage, ' + dataset + ', ' + str(max_iter) + ' iterations, m = ' + str(episode_max) + ', sample ' + str(k) + ' .txt')
